...
const ApiFactory = newClass(
  class {
    constructor(params, bypass = false) {
      ...
      this.templates = {
        profile: this.constructor.templates.profile
      };
      this.template = this.templates.profile;
    ...
    get endpoint() {
      return this.constructor.createEndpoint(this.params);
    }
    static get listName() {
    ...
    static viewProps(params) {
      ...
      if (this.getProfiles)
        vps.hooks.execute = async function (hook) {
          ...
          if (hook) await hook.call(this, params, profiles.error);
        }.bind(vps);
...
class View {
  constructor(factory, params) {
    ...
    this.session = {
      get() {
        return View.storage.get("session");
      },
      save(par) {
        return View.storage.set("session", par || this.params);
      }
    };
    this.__animation = {
      style: "opacity",
    ...
    this.hooks = {
      loading: function () {
        ...
        if (this.html.loading.style)
          Object.assign(child.style, this.html.loading.style);
  ...
  set parent(parent) {
    ...
    let parentElement = (this.__parentElement = document.getElementById(
      parent + "s"
    ));
  ...
  async execute(params, cb) {
    ...
    if (!this.component) this.component = list.component;
    ...
    if (props.data) this.children = props.data;

    if (props.html && this.html) Object.assign(this.html, props.html);
    ...
    setTimeout(() => {
      ...
      if (this.hooks.loading && this.hooks.loading.undo)
        this.hooks.loading.undo();
    }, time);
  }
  get html() {
    return this.__html;
  ...
  set animation(an) {
    if (an === false) this.__animation.time = 0;
...
View.Element = class Element {
  ...
  render(props) {
    let self = this;
    if (typeof props === "object") Object.assign(this, props);
    if (this.template && this.vars) this.markup = this.template(this.vars);
...
View.storage = {
  set: (key, val) => {
    try {
      ...
      console.error("local storage not enabled");
    }
  },
  get: (key, val) => {
    try {
...
function newInstance(instance) {
  Reflect.ownKeys(instance).forEach((key) => {
    ...
    if (key.indexOf("__") === 0)
      ...
      Object.defineProperty(instance, key, { ...desc, enumerable: true });
  });
  return instance;
}
module.exports = { ApiFactory, View };
